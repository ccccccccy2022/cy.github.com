...
class PreProcess(nn.Module):
    ...
    def __init__(self, input_size, processed_x_size):
        ...
        self.input_size = input_size
        self.Ln=nn.LayerNorm(input_size, eps=1e-05)
        self.fc1 = nn.Linear(input_size, processed_x_size)
        self.fc2 = nn.Linear(processed_x_size, processed_x_size)

    def forward(self, input):
        t = torch.relu(self.fc1(self.Ln(input)))
...
class TD_LSTM(nn.Module):
    def __init__(self,input_size, hidden_size):
        ...
        self.LSTM=nn.LSTM(input_size=self.input_size,
                               ...
                               batch_first=True)
        self.mu=nn.Linear(1,self.hidden_size)
        self.final=nn.Linear(self.hidden_size,1)

    def forward(self,inputs,yt_1):
        inputs=self.LSTM(inputs)[1][0].squeeze(0)   #[128,20,400]   [128,20,20]
        y=self.mu(torch.tensor(yt_1,dtype=torch.float32))  #[128,20]
        y2=torch.sigmoid(inputs)*y
        inputs=self.final(torch.relu(y2))
        return inputs
...
class TD_VAE(nn.Module):
    ...
    """
    def __init__(self, x_size,batch_size, processed_x_size, b_size, z_size):
        super(TD_VAE, self).__init__()
        self.x_size = x_size
        self.processed_x_size = processed_x_size
        self.b_size = b_size   #belief_state_size
        self.z_size = z_size   #state_size
        ...
        self.decoder = nn.LSTM(input_size=self.x_size,
                               hidden_size=1,
                               batch_first=True).cuda()
        self.decoder2=TD_LSTM(input_size=self.x_size,hidden_size=self.b_size)
    ...
    def calculate_loss(self, t1, t2):
        ...
        ## infer state at time t1 based on states at time t2  qs=(Zt1|St1,St2,Zt2)
        ...
        ## state distribution at time t2 based on states at time t1 and state transition  #PT(zt2)t2  p(St2|St1)
        ...
        t2_x_prob = self.z_to_x(t2_z)  #PD(xt2)t2    P(x|z)
    ...
    def rollout(self, images, t1, t2,yt_1):
        self.forward(images)
        
        ## at time t1-1, we sample a state z based on belief at time t1-1
        l2_z_mu, l2_z_logsigma = self.l2_b_to_z(self.b[:,t1-1,:])   #self.b=[6,20,10]
        l2_z_epsilon = torch.randn_like(l2_z_mu)
        l2_z = l2_z_mu + torch.exp(l2_z_logsigma)*l2_z_epsilon #l2_z [6,8]
        ...
        for k in range(t2 - t1 + 1):
            ...
            next_l1_z_epsilon = torch.randn_like(next_l1_z_mu)      #[6,8]   
            ...
            next_x = self.z_to_x(next_z)  #[6,20]
            rollout_x.append(next_x)
            current_z = next_z   #[6,16]

        rollout_x = torch.stack(rollout_x, dim = 1) #[6,8,20]
        rollout_x2=self.decoder2(rollout_x,yt_1)
        return rollout_x,rollout_x2
    def Decoder2(self,images,yt_1,y):
        images=images.cuda()
        # output,(h_n,c_n)=self.decoder(images)
        h_n=self.decoder2(images,yt_1)
        criteria = nn.MSELoss()
        loss = criteria(h_n.flatten(),y.flatten())
        return loss,h_n